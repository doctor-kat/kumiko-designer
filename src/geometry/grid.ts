...
export function generatePanelGeometry(
    ...
    for (let row = 0; row < rows; row++) {
        for (let col = 0; col < cols; col++) {
            ...
            const segments = generatePatternSegments(pattern, tri, rotation, pattern.baseWeight);
...
function clipAndAddSegments(
    ...
    // Clip lines (preserve intersection weight metadata)
    for (const line of segments.lines) {
        ...
        if (clipped) {
            result.lines.push({
                ...clipped,
                weight: line.weight,
                startIntersectWeight: line.startIntersectWeight,
                endIntersectWeight: line.endIntersectWeight,
            });
        }
    }
...
export function generateTiledPreview(
    ...
    for (let row = 0; row < 3; row++) {
        for (let col = 0; col < 4; col++) {
            ...
            const segments = generatePatternSegments(pattern, tri, 0, pattern.baseWeight);
